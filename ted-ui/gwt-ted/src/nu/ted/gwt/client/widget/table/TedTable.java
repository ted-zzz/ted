...
import nu.ted.gwt.client.Css.Widgets;
...
public abstract class TedTable<T> extends Grid implements TedTableModelListener, ClickHandler {
	...
	public TedTable(TedTableModel<T> model) {
		setStyleName(Widgets.TED_SERIES_TABLE);
		this.model = model;
	...
	public void setSelectedRow(int rowToSelect) {
		...
		if (this.selectedRow >= 0) {
			getRowFormatter().addStyleName(this.selectedRow, Widgets.TED_TABLE_ROW_SELECTED);
		}

		if (oldSelection >= 0) {
			getRowFormatter().removeStyleName(oldSelection, Widgets.TED_TABLE_ROW_SELECTED);
		}
	...
	private void buildTableFromModel() {
		...
		for (T next : this.model.getData()) {
			...
			getRowFormatter().setStyleName(row, Widgets.TED_TABLE_ROW);
			if (row % 2 == 0) {
				getRowFormatter().setStyleName(row, Widgets.TED_TABLE_ROW_ALTERNATE);
			}
